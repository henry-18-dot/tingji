...
import { getNoteResource, parseNoteImageResources } from './note-image-registry.js';
...
export function appendInline(parent, text) {
  ...
  for (const match of String(text).matchAll(tokens)) {
    ...
    if (match[2] !== undefined) parent.append(math(match[2], true, ['$$', '$$']));
      ...
      } else if (/^https?:\/\//i.test(match[10]) || /^\/?\?note=[a-zA-Z0-9-]+(?:#[a-zA-Z0-9_-]+)?$/.test(match[10])) {
        const link = node('a', '', match[9]);
        link.href = match[10];
        if (/^https?:\/\//i.test(match[10])) { link.target = '_blank'; link.rel = 'noopener noreferrer'; }
        parent.append(link);
...
function cells(line) {
  ...
  const result = []; let cell = '', code = false, math = null;
  for (let i = 0; i < value.length; i++) {
    const char = value[i];
    if (char === '\\' && value[i + 1] === '|') { cell += '|'; i++; }
    else if (!code && char === '\\' && value[i + 1] === '(') { math = '\\)'; cell += '\\('; i++; }
    else if (!code && math === '\\)' && char === '\\' && value[i + 1] === ')') { math = null; cell += '\\)'; i++; }
    else if (char === '`') { code = !code; cell += char; }
    else if (!code && char === '$' && value[i - 1] !== '\\') {
      const delimiter = value[i + 1] === '$' ? '$$' : '$';
      if (math === delimiter) math = null;
      else if (!math && (delimiter === '$$' ? /(?<!\\)\$\$/ : /(?<!\\)\$/).test(value.slice(i + delimiter.length))) math = delimiter;
      cell += delimiter; i += delimiter.length - 1;
    }
    else if (char === '|' && !code && !math) { result.push(cell.trim()); cell = ''; }
    else cell += char;
  }
...
function resourceImage(description, destination, resources) {
  const resource = getNoteResource(destination, resources);
  if (!resource) return null;
  const path = resource.imagePath;
  const figure = node('figure', 'knowledge-image');
  const link = node('a', 'knowledge-image-link');
  link.href = path; link.target = '_blank'; link.rel = 'noopener noreferrer';
  link.setAttribute('aria-label', `${description || resource.title}，打开原图`);
  const image = node('img');
  image.src = path; image.alt = description || resource.title;
  image.loading = 'lazy'; image.decoding = 'async'; image.dataset.noteResource = path;
  image.referrerPolicy = 'no-referrer';
  if (resource.width && resource.height) { image.width = resource.width; image.height = resource.height; }
  link.append(image);
  const caption = node('figcaption');
  if (description) caption.append(node('span', 'knowledge-image-description', description));
  const attribution = node('span', 'knowledge-image-attribution');
  const source = node('a', '', `${resource.author}${resource.sourceYear ? ` · ${resource.sourceYear}` : ''}${resource.page ? ` · 第 ${resource.page} 页` : ''}`);
  source.href = resource.url; source.target = '_blank'; source.rel = 'noopener noreferrer';
  const license = node('a', '', resource.license);
  license.href = resource.licenseUrl; license.target = '_blank'; license.rel = 'noopener noreferrer';
  attribution.append(source, document.createTextNode(' · '), license);
  if (resource.credit) attribution.append(document.createTextNode(` · ${resource.credit}`));
  caption.append(attribution);
  figure.append(link, caption);
  image.addEventListener('error', () => { link.replaceWith(node('p', 'knowledge-image-error', '图像暂时无法加载，可打开下方来源。')); }, { once: true });
  return figure;
}

export function renderMarkdown(text, target, options = {}) {
  target.readingCleanup?.();
  const imageResources = parseNoteImageResources(String(text || '').replace(/\r\n?/g, '\n'));
  ...
  const render = (lines, components = true) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Raw SVG from older generated notes uses the same inert allowlist as fenced SVG.
      if (/^\s*<svg(?:\s|>)/i.test(line)) {
        let end=i; while(end<lines.length && end-i<600 && !/<\/svg>\s*$/i.test(lines[end])) end++;
        if(end<lines.length) { const diagram=svgDiagram(lines.slice(i,end+1).join('\n')); if(diagram){flush();fragment.append(diagram);i=end;continue;} }
      }
      ...
      if (opening) {
        ...
        if (language === 'note-images') continue;
      ...
      if (directive) {
        ...
        if (end < lines.length) {
          ...
          if (directive[1] === 'details') {
            if (/^(?:课程信息|来源定位|待核对|待确定)$/.test(directive[2].trim())) { i = end; continue; }
      ...
      if (image) {
        flush();
        const figure = resourceImage(image[1], image[2], imageResources);
        fragment.append(figure || node('p', '', image[1] || '图片'));
        continue;
      }
  ...
  // Bundled images already render their source and licence in the caption.
  // Keep textual attribution in Markdown exports, but show it once here.
  for (const figure of fragment.querySelectorAll('.knowledge-image')) {
    const destination = figure.querySelector('img[data-note-resource]')?.dataset.noteResource;
    const resource = destination && getNoteResource(destination, imageResources);
    let sibling = figure.nextElementSibling;
    for (let distance = 0; resource && sibling?.tagName === 'P' && distance < 2; distance++) {
      const next = sibling.nextElementSibling;
      if (/^来源[：:]/.test(sibling.textContent.trim()) && [...sibling.querySelectorAll('a')].some(link => link.href === resource.url)) sibling.remove();
      sibling = next;
    }
  }
  if (headings.length) {
  ...
  const documentTitle = [...fragment.childNodes].find(child => child.nodeName === 'H1');
  if (documentTitle) {
    const whole = node('details', 'knowledge-whole'), summary = node('summary'), content = node('div', 'knowledge-whole-body');
    whole.open = true; documentTitle.before(whole); summary.append(documentTitle); whole.append(summary, content);
    let sibling = whole.nextSibling;
    while (sibling) { const next = sibling.nextSibling; content.append(sibling); sibling = next; }
  }
