...
const knowledgeIndexes = concept => {
  const seen = new Set(), links = [], query = encodeURIComponent(concept.term.trim());
  const supplied = Array.isArray(concept.links) ? concept.links : [];
  for (const item of [...supplied,
    { title: `知乎搜索：${concept.term}`, url: `https://www.zhihu.com/search?type=content&q=${query}` },
    { title: `B站搜索：${concept.term}`, url: `https://search.bilibili.com/all?keyword=${query}` },
    { title: `网页搜索：${concept.term}`, url: `https://www.bing.com/search?q=${query}` }]) {
    const title = String(item?.title || '').trim(), url = safeUrl(item?.url);
    if (title && url && !seen.has(url)) { links.push({ title, url }); seen.add(url); }
...
export function mountReadingFeatures(root, data = {}) {
  ...
  for (const trigger of root.querySelectorAll('.concept-trigger')) {
    ...
    trigger.addEventListener('click', () => {
      ...
      const links = knowledgeIndexes(concept);
      if (links.length) {
  ...
  for (const heading of root.querySelectorAll('h2,h3')) {
    if (!['继续探索', '额外探索'].includes(heading.textContent.trim())) continue;
