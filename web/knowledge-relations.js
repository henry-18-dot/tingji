...
function parseFlowchart(source) {
  if (source.length > 5000) throw new Error('关系图请控制在 12 个节点以内。');
  if (/%%\s*\{|[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(source)) {
  ...
  function node() {
    skipSpace();
    const match = /^[A-Za-z_][A-Za-z0-9_]{0,47}/.exec(input.slice(position));
    if (!match || /^(?:click|href|callback|link|linkStyle|style|classDef|class|subgraph|end|direction)$/i.test(match[0])) {
      throw new Error('关系图只支持节点和连线；节点编号请使用英文字母或下划线开头。');
    }
    const id = match[0];
    position += id.length;
    skipSpace();
    ...
    if (nodes.size > 12) throw new Error('关系图最多显示 12 个节点，请拆成几幅小图。');
